    ...
    if existing:
        if (
            ...
            cluster_mgr.module.fail_json(
                msg=f"Unable to initialize the CloudHSM Cluster '{existing[0]['ClusterId']}'. \
                    The selected cluster must be in 'UNINITIALIZED' state. Reported state is '{existing[0]['State']}'"
            )
        elif not existing[0]["Certificates"]:
            cluster_mgr.module.fail_json(
                msg=f"Unable to initialize the CloudHSM Cluster '{existing[0]['ClusterId']}'. \
                    The selected cluster does not have any HSM devices."
            )
    else:
