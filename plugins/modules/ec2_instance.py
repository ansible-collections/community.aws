...
def ensure_instance_state(desired_module_state, ec2=None):
    if ec2 is None:
        module.client('ec2', retry_decorator=AWSRetry.jittered_backoff())
    if desired_module_state in ('running', 'started'):
        changed, failed, instances, failure_reason = change_instance_state(filters=module.params.get('filters'), desired_module_state=desired_module_state)
...
def change_instance_state(filters, desired_module_state, ec2=None):
    if ec2 is None:
        ec2 = module.client('ec2', retry_decorator=AWSRetry.jittered_backoff())
