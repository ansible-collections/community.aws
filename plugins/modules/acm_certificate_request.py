...
def main():
    argument_spec = dict(
        certificate_authority_arn=dict(aliases=['carn']),
        domain_name =dict(),
        idempotency_token=dict(),
        validation_method=dict(type='str', choices=['EMAIL', 'DNS']),
        subject_alternative_names=dict(type='list', elements='str'),
        tags=dict(type='list', elements='dict'),
        key_algorithm=dict(type='str', choices=['RSA_1024', 'RSA_2048', 'RSA_3072', 'RSA_4096', 'EC_prime256v1', 'EC_secp384r1', 'EC_secp521r1'])
    )
    ...
    if "domain_name" not in module.params:
      module.exit_json(error="Domain name required")

    certificates = acm_info.generate_certificate(client, module,
      domain_name=module.params['domain_name'],
      certificate_authority_arn=module.params['certificate_authority_arn'],
      ...
      tag_list=module.params['tags'],
      key_algorithm=module.params['key_algorithm'])

    module.exit_json(certificate=certificates)
